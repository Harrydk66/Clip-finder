...
export async function runExperiment(snapshot,{model='gpt-4o-mini',cache={},save=async()=>{},judge,apiKey}={}) {
  const result = {version:VERSION,model,rubricHash:digest(RUBRIC),snapshotHash:digest(snapshot),snapshot,cache,
    status:'running',createdAt:new Date().toISOString(),evidencePolicy:'unsupported-axes-null-v1',invalidResponsePolicy:'unavailable-cached-v1',rankingPolicy:'decision, mean of known cold axes, old rank; eventValue excluded',
